...
if command -v yamllint >/dev/null 2>&1; then
  ...
  for path in \
    ...
    "${PROJECT_ROOT}/roles" \
    "${PROJECT_ROOT}/vars"; do
    [[ -e "${path}" ]] && YAMLLINT_PATHS+=("${path}")
  done
  if [[ ${#YAMLLINT_PATHS[@]} -gt 0 ]]; then
    if [[ -f "${PROJECT_ROOT}/.yamllint" ]]; then
      YAMLLINT_ARGS=(-c "${PROJECT_ROOT}/.yamllint")
    fi
    yamllint "${YAMLLINT_ARGS[@]}" "${YAMLLINT_PATHS[@]}"
  else
    echo "no yamllint targets found under ${PROJECT_ROOT}; skipping"
  fi
else
...
if command -v ansible-lint >/dev/null 2>&1; then
  ...
  ANSIBLE_LINT_ARGS=(
    --exclude "${PROJECT_ROOT}/../aws-metal-openshift-demo"
  )
  ANSIBLE_LINT_TARGETS=("${TOP_LEVEL_PLAYBOOKS[@]}")
  if [[ -d "${PROJECT_ROOT}/roles" ]]; then
    ANSIBLE_LINT_TARGETS+=("${PROJECT_ROOT}/roles")
  fi
  if ! ansible-lint "${ANSIBLE_LINT_ARGS[@]}" "${ANSIBLE_LINT_TARGETS[@]}"; then
...
if [[ -f "${PRECHECK_PLAYBOOK}" ]]; then
  ...
  echo "==> Orchestration preflight playbook not present in this project; skipping"
fi

if [[ -f "${SCOPED_CONTRACTS_PLAYBOOK}" ]]; then
  ...
  echo "==> Scoped play contract checks not present in this project; skipping"
fi
