#!/usr/bin/env bash
# lab-dashboard.sh — tmux-based operator dashboard for the Calabi on-prem lab.
#
...
#   SESSION        — tmux session name (default: lab-dashboard-onprem)
...
REFRESH="${REFRESH:-10}"
SESSION="${SESSION:-lab-dashboard-onprem}"
SCRIPT_PATH="$(readlink -f "${BASH_SOURCE[0]}" 2>/dev/null || printf '%s' "${BASH_SOURCE[0]}")"
PROJECT_ROOT_CANDIDATES=(
  "$(cd "$(dirname "$SCRIPT_PATH")/.." && pwd)"
  "/opt/openshift/on-prem-openshift-demo"
  "${HOME}/codex/calabi-upstream/on-prem-openshift-demo"
)
PROJECT_ROOT=""
...
if [[ -d /opt/openshift/on-prem-openshift-demo && -r "$STAGED_PROJECT_KEY" ]]; then
  DASHBOARD_MODE="bastion"
  VIRT_HOST_DEFAULT="172.16.0.1"
  VIRT_USER_DEFAULT="ec2-user"
  SSH_KEY_DEFAULT="$STAGED_PROJECT_KEY"
  STATE_DIR="${BASTION_STATE_DIR}"
else
  DASHBOARD_MODE="workstation"
  SSH_KEY_DEFAULT=""
  for candidate in "$LOCAL_PROJECT_KEY" "$STAGED_PROJECT_KEY" "$SHARED_PROJECT_KEY"; do
    if [[ -r "$candidate" ]]; then
...
SSH_KEY="$(expand_user_path "${SSH_KEY}")"
...
if [[ ! -r "$SSH_KEY" ]]; then
  ...
  echo "" >&2
  echo "  Use the staged on-prem project key or a per-user 0600 copy" >&2
  echo "  of the hypervisor key instead of making the shared private key" >&2
  echo "  group-readable:" >&2
  echo "    sudo install -d -m 700 \$HOME/.ssh" >&2
  echo "    sudo install -m 600 -o $(whoami) -g $(id -gn) \\" >&2
  echo "      /opt/openshift/on-prem-openshift-demo/secrets/id_ed25519 \\" >&2
  echo "      \$HOME/.ssh/lab-hypervisor.key" >&2
  echo "    SSH_KEY=\$HOME/.ssh/lab-hypervisor.key /usr/local/bin/lab-dashboard-onprem" >&2
  exit 1
fi
...
printf 'building\n' > "$MANIFEST_STATUS"
cd "$PROJECT_ROOT"
if ANSIBLE_COLLECTIONS_PATHS="${ANSIBLE_COLLECTIONS_PATHS:-${HOME}/.ansible/collections:/usr/share/ansible/collections:/usr/local/share/ansible/collections}" \
  ansible-playbook -i inventory/hosts.yml "$PLAYBOOK_PATH" --list-tasks 2>"$LISTTASKS_ERR" | build_from_stream; then
  printf 'ready\n' > "$MANIFEST_STATUS"
  exit 0
fi

if bastion_ssh_manifest "cd /opt/openshift/on-prem-openshift-demo && ANSIBLE_COLLECTIONS_PATHS=\"\${ANSIBLE_COLLECTIONS_PATHS:-/usr/share/ansible/collections:/usr/local/share/ansible/collections}\" ansible-playbook -i inventory/hosts.yml '$PLAYBOOK_PATH' --list-tasks 2>/dev/null" | build_from_stream; then
